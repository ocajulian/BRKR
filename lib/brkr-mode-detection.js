...
function isPureGreeting(text) {
  const greetings = getGreetingTerms();
...
export function detectOnboardingState({ message, history = [] }) {
  ...
  const previousUserTurns = Array.isArray(history)
    ? history.filter(
        (m) =>
          m &&
          m.role === "user" &&
  ...
  const looksLikeGreeting =
    isPureGreeting(current) ||
    (hasAny(current, getGreetingTerms()) && words <= 5);

  const looksVague =
    words <= 5 &&
    !hasBusinessSignal(current) &&
    (looksLikeGreeting || hasAny(current, vagueHelpTerms));

  if (previousUserTurns === 0 && (looksLikeGreeting || looksVague)) {
    return "WELCOME";
  }
